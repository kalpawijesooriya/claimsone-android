...
public class ColorExpListAdapterBus extends BaseExpandableListAdapter {
	Logger LOG = LoggerFactory.getLogger(ColorExpListAdapterBus.class);

	
	public ColorExpListAdapterBus(Context context,
		...
		try {
			...
			LOG.error("ColorExpListAdapterBus:10207");
			   if(e != null){
	...
	public View getChildView(int groupPosition, int childPosition,
		...
		try {
			//LogFile.d(LOG_TAG, "getChildView: groupPositon: " + groupPosition
			...
			if (listViewCache[groupPosition] != null)
				...
				dev.setAdapter(new DebugSimpleExpandableListAdapter(context,
						groupPosition, createGroupList(groupPosition), // groupData describes the first-level entries
																		
						R.layout.multilevel_expandable_list_group_2_row, // Layout for the first-level entries
																		
						new String[] { KEY_COLORNAME }, // Key in the groupData maps to display
														
						new int[] { R.id.group2name }, // Data under "colorName" key goes into this TextView
													
						createChildList(groupPosition), // childData describes second-level entries
														
						R.layout.multilevel_expandable_list_child3_row, // Layout for second-level entries
	...
	private List createGroupList(int level1) {
		...
		try {
			...
			LOG.error("createGroupList:10208");
			   if(e != null){
				LOG.error("Message: " + e.getMessage());
				LOG.error("StackTrace: " + e.getStackTrace());
			  }
	...
	private List createChildList(int level1) {
		...
		try {
			...
			LOG.error("createChildList:10209");
			   if(e != null){
				LOG.error("Message: " + e.getMessage());
				LOG.error("StackTrace: " + e.getStackTrace());
			  }
	...
	class Level2ChildExpandListener implements
		...
		public Level2ChildExpandListener(int level1GroupPosition) {
			...
			//LogFile.d("\n\n", "Only Hope-\n\n" + level1GroupPosition + "-" + listdesc.length + "-" + listdesc[0].length);
			GroupClass1 gc;
			try{
			for (int i = 0; i < 1; i++) {
				 gc= DebugSimpleExpandableListAdapter.buttonarraylist[level1GroupPosition][1][1];
				 CheckBox cb = gc.cb;
				 cb.setChecked(false);
			}
			}catch(Exception e){
				LOG.error("Level2ChildExpandListener:10210");
				   if(e != null){
					LOG.error("Message: " + e.getMessage());
					LOG.error("StackTrace: " + e.getStackTrace());
				  }
			...
			try {
				...
				if (!formObject.isEditable()) {
					...
					if(formObject.getisPreSelected()){  //PRE
						if (cb.isChecked()) {
							cb.setChecked(false);
							ExpList3_Bus.booleanprelistBus[level1GroupPosition][groupPosition][childPosition][0] = false;
							//LogFile.d("\n\n", "FALSE:" + groupPosition +"-"+ childPosition);
						} else {
							cb.setChecked(true);
							ExpList3_Bus.booleanprelistBus[level1GroupPosition][groupPosition][childPosition][0] = true;
							//LogFile.d("\n\n", "TRUE:" + groupPosition +"-"+ childPosition);
						}
					} else {
						if (cb.isChecked()) {
							cb.setChecked(false);
							ExpList3_Bus.booleanlistBus[level1GroupPosition][groupPosition][childPosition][0] = false;
						} else {
							cb.setChecked(true);
							ExpList3_Bus.booleanlistBus[level1GroupPosition][groupPosition][childPosition][0] = true;
						}
					}
				}
				return false;
			} catch (Exception e) {
				LOG.error("onChildClick:10211");
				   if(e != null){
					LOG.error("Message: " + e.getMessage());
					LOG.error("StackTrace: " + e.getStackTrace());
				  }
