...
public class ColorExpListAdapterCar extends BaseExpandableListAdapter {

	Logger LOG = LoggerFactory.getLogger(ColorExpListAdapterCar.class);

	
	public ColorExpListAdapterCar(Context context,
	...
	public View getGroupView(int groupPosition, boolean isExpanded,
		...
		try {
			...
			if (convertView != null)
				...
				v = inflater.inflate(R.layout.multilevel_expandable_list_group_row,
						parent, false);
	...
	private List createGroupList(int level1) {
		...
		try {
			...
			LOG.error("createGroupList:10212");
			   if(e != null){
				LOG.error("Message: " + e.getMessage());
				LOG.error("StackTrace: " + e.getStackTrace());
			  }
	...
	private List createChildList(int level1) {
		...
		try {
			...
			LOG.error("createGroupList:10213");
			   if(e != null){
				LOG.error("Message: " + e.getMessage());
				LOG.error("StackTrace: " + e.getStackTrace());
			  }
	...
	class Level2ChildExpandListener implements
		...
		public Level2ChildExpandListener(int level1GroupPosition) {
			...
			for (int i = 0; i < 1; i++) {
				...
				LOG.error("Level2ChildExpandListener:10215");
				   if(e != null){
					LOG.error("Message: " + e.getMessage());
					LOG.error("StackTrace: " + e.getStackTrace());
				  }
			...
			try {
				...
				if (!formObject.isEditable()) {
					...
					if(formObject.getisPreSelected()){  //PRE
						if (cb.isChecked()) {
							cb.setChecked(false);
							ExpList3_Car.booleanprelistCar[level1GroupPosition][groupPosition][childPosition][0] = false;
						} else {
							cb.setChecked(true);
							ExpList3_Car.booleanprelistCar[level1GroupPosition][groupPosition][childPosition][0] = true;
						}
					}
					else {
					if (cb.isChecked()) {
						cb.setChecked(false);
						ExpList3_Car.booleanlistCar[level1GroupPosition][groupPosition][childPosition][0] = false;
					} else {
						cb.setChecked(true);
						ExpList3_Car.booleanlistCar[level1GroupPosition][groupPosition][childPosition][0] = true;
					}
					}
				}
				return false;
			} catch (Exception e) {
				LOG.error("onChildClick:10216");
				   if(e != null){
					LOG.error("Message: " + e.getMessage());
					LOG.error("StackTrace: " + e.getStackTrace());
				  }
