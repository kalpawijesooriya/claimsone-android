...
public class ExpList3_Van extends Activity implements
	...
	static final String freqDamagedItemMapper [] = {"1/0", "1/11", "1/12", "1/13", "1/14", "1/15", "1/16", "1/17", "1/18",
									"4/0", "4/11", "4/12", "4/13", "4/14", "4/15", "4/16", "4/17"};
	static final String listdesc[][][][] = { { // grey
			{
				{ "Van", "Frequently damaged items" },
					{ "Front buffer", "" },
					...
					{ "L/H/S rear parking lamp", "" },{ "R/H/S rear buffer end", "" },
			...
			{ "Van", "Rear side" }, { "Rear buffer", "" },
					...
					{ "L/H/S tail light rear", "" },//10
					{ "R/H/S Rear tail light", "" },
					{ "L/H/S Rear tail light", "" },
					{ "R/H/S rear parking lamp", "" },
					{ "L/H/S rear parking lamp", "" },
					{ "R/H/S rear buffer end", "" },
					{ "L/H/S rear buffer end", "" }, { "Dickey door", "" },//17
					{ "Dickey door lights", "" },
					...
					for (int j = 0; j < listdesc[i].length; j++) {
						...
						for (int k = 0; k < listdesc[i][j].length -1; k++) {
							...
							if(j==0){
								...
								SubPart subPart = new SubPart(j+"/"+k, listdesc[i][j][k+1][0], booleanprelistVan[0][x-1][y][0], listdesc[i][j][k+1][1]);
								subParts.add(subPart);
							} else {
								SubPart subPart = new SubPart(j+"/"+k, listdesc[i][j][k+1][0],booleanprelistVan[i][j-1][k][0], listdesc[i][j][k+1][1]);
								subParts.add(subPart);
							}
						}
					...
					for (int j = 0; j < listdesc[i].length; j++) {
						...
						for (int k = 0; k < listdesc[i][j].length -1; k++) {
							...
							if(j==0){
								String[] indexValues = freqDamagedItemMapper[k].split("/");
								int x = Integer.parseInt(indexValues[0]); int y = Integer.parseInt(indexValues[1]);
								SubPart subPart = new SubPart(j+"/"+k, listdesc[i][j][k+1][0], booleanlistVan[0][x-1][y][0], listdesc[i][j][k+1][1]);
								subParts.add(subPart);
							} else {
								SubPart subPart = new SubPart(j+"/"+k, listdesc[i][j][k+1][0],booleanlistVan[i][j-1][k][0], listdesc[i][j][k+1][1]);
								subParts.add(subPart);
							}
						}
								...
								if (checkBox.isChecked()) { //if is already checked
									...
									for(int i =1; i<listdesc[0].length; i++){
										for(int j = 1; j<listdesc[0][i].length; j++){
											...
											if (text.equals(text2)){
												if(i != 0 && j!=0) {ExpList3_Van.booleanprelistVan[0][i-1][j-1][0] = false;}
											}
										}
									}
									...
									for(int i =1; i<listdesc[0].length; i++){
										for(int j = 1; j<listdesc[0][i].length; j++){
											String text2 = listdesc[0][i][j][0];
											if (text.equals(text2)){
												if(i != 0 && j!=0) {ExpList3_Van.booleanprelistVan[0][i-1][j-1][0] = true;}
											}
										}
									}
								...
								if (checkBox.isChecked()) { //if is already checked
									...
									for(int i =1; i<listdesc[0].length; i++){
										for(int j = 1; j<listdesc[0][i].length; j++){
											String text2 = listdesc[0][i][j][0];
											if (text.equals(text2)){
												if(i != 0 && j!=0) {ExpList3_Van.booleanlistVan[0][i-1][j-1][0] = false;}
											}
										}
									}
									listAdapter.checkForDoubleItems(text, false);
									...
									for(int i =1; i<listdesc[0].length; i++){
										for(int j = 1; j<listdesc[0][i].length; j++){
											String text2 = listdesc[0][i][j][0];
											if (text.equals(text2)){
												if(i != 0 && j!=0) {ExpList3_Van.booleanlistVan[0][i-1][j-1][0] = true;}
											}
										}
									}
									listAdapter.checkForDoubleItems(text, true);
	...
	private void displayList() {
		...
		listAdapter = new MyListAdapter(ExpList3_Van.this, mainPartsList);
