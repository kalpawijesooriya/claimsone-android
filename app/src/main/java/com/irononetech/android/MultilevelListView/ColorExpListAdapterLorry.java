...
public class ColorExpListAdapterLorry extends BaseExpandableListAdapter {
	Logger LOG = LoggerFactory.getLogger(ColorExpListAdapterLorry.class);

	
	public ColorExpListAdapterLorry(Context context,
			...
			try {
				...
				if (!formObject.isEditable()) {
					...
					if(formObject.getisPreSelected()) {
						if (cb.isChecked()) {
							cb.setChecked(false);
							ExpList3_Lorry.booleanprelistLorry[level1GroupPosition][groupPosition][childPosition][0] = false;
						} else {
							cb.setChecked(true);
							ExpList3_Lorry.booleanprelistLorry[level1GroupPosition][groupPosition][childPosition][0] = true;
						}
					}
					else {
					if (cb.isChecked()) {
						cb.setChecked(false);
						ExpList3_Lorry.booleanlistLorry[level1GroupPosition][groupPosition][childPosition][0] = false;
					} else {
						cb.setChecked(true);
						ExpList3_Lorry.booleanlistLorry[level1GroupPosition][groupPosition][childPosition][0] = true;
					}
					}
				}
				return false;
